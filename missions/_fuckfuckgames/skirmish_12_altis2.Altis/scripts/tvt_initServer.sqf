...
if (!(blufor countSide allUnits isEqualTo 0)) then {
	[ "itemAdd", [ 
		...
		1, "seconds", { opfor countSide allUnits isEqualTo 0 && {independent countSide allUnits isEqualTo 0 && {civilian countside allUnits isEqualTo 0}} }
	] ] call BIS_fnc_loop; 
};
if (!(opfor countSide allUnits isEqualTo 0)) then {
	[ "itemAdd", [ 
		...
		1, "seconds", { blufor countSide allUnits isEqualTo 0 && {independent countSide allUnits isEqualTo 0 && {civilian countside allUnits isEqualTo 0}} }
	] ] call BIS_fnc_loop; 
};
if (!(independent countSide allUnits isEqualTo 0)) then {
	[ "itemAdd", [ 
		...
		1, "seconds", { blufor countSide allUnits isEqualTo 0 && {opfor countSide allUnits isEqualTo 0 && {civilian countside allUnits isEqualTo 0}} }
	] ] call BIS_fnc_loop; 
};
if (!(civilian countSide allUnits isEqualTo 0)) then {
	[ "itemAdd", [ 
		"checkCivilianWins", {
			...
			opfor call sxf_fnc_handleTeamLoses;		
			independent call sxf_fnc_handleTeamLoses;
			civilian call sxf_fnc_handleTeamWins;
			"SideScore" call BIS_fnc_endMissionServer;
			["itemRemove", ["checkCivilianWins"]] call BIS_fnc_loop;
		}, 
		1, "seconds", { blufor countSide allUnits isEqualTo 0 && {opfor countSide allUnits isEqualTo 0 && {independent countside allUnits isEqualTo 0}} }
	] ] call BIS_fnc_loop; 
};
...
sxf_fnc_loopMessage = {	
	...
	if (count _civilianList != 0) then {
		_livingPlayersMessage = _livingPlayersMessage + "<br/><br/>" + "> CIVILIAN" + "<br/>";
		{ _livingPlayersMessage = _livingPlayersMessage + (_x call sxf_fnc_getPlayerStatusText); } forEach _civilianList;
	};
