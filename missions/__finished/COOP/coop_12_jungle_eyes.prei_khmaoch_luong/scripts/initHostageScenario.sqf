...
_tempHostageList = ["hostage_1"];
_tempExtractionZones = ["trg_extractionZone_1"];
...
if ({isPlayer _x} count sv_hostageList > 0) exitWith {};
			...
			[
				...
				[
					"Carry the hostage",
					{
						...
						_bCarried = _theHostage getVariable ["bCarried", false];
						if (_bCarried) then 
						{	//drop the hostage		
